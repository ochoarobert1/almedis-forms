let passd = true,
  ...
  almedisUserTab = document.getElementsByClassName(
    "almedis-user-dashboard-tab"
  ),
...
if (downloadXLS) {
  downloadXLS.addEventListener("click", function (e) {
    ...
    resultDiv.innerHTML =
      '<div class="loader-modal"><div class="loader"></div></div>';
...
if (passwordShow.length > 0) {
  for (var i = 0; i < passwordShow.length; i++) {
    passwordShow[i].addEventListener(
      "click",
      function (e) {
        currentState = this.previousElementSibling.getAttribute("type");
        if (currentState == "password") {
          currentState = this.previousElementSibling.setAttribute(
            "type",
            "text"
          );
        } else {
...
if (convenioSingleSubmit) {
  convenioSingleSubmit.addEventListener("click", function (e) {
    ...
    if (passd == true) {
      ...
      resultDiv.innerHTML = '<div class="loader"></div>';
      var formData = new FormData(
        document.getElementById("almedisConveniosSingleForm")
      );
...
if (registroSubmit) {
  registroSubmit.addEventListener("click", function (e) {
    ...
    if (passd == true) {
      ...
      resultDiv.innerHTML = '<div class="loader"></div>';
      var formData = new FormData(
        document.getElementById("almedisRegistroForm")
      );
      ...
      newRequest.onload = function () {
        var result = JSON.parse(newRequest.responseText);
        resultDiv.innerHTML =
          '<span class="response-text">' + result.data.message + "</span>";
...
if (institucionSubmit) {
  institucionSubmit.addEventListener("click", function (e) {
    ...
    if (passd == true) {
      ...
      resultDiv.innerHTML = '<div class="loader"></div>';
      var formData = new FormData(
        document.getElementById("updateInstitucionData")
      );
...
if (almedisLoginSubmit) {
  almedisLoginSubmit.addEventListener(
    "click",
    function (e) {
      e.preventDefault();
      var elem = "";
      ...
      if (passd == true) {
        ...
        resultDiv.innerHTML = '<div class="loader"></div>';
        var formData = new FormData(
          document.getElementById("almedisLoginForm")
        );
...
if (almedisLostSubmit) {
  almedisLostSubmit.addEventListener(
    "click",
    function (e) {
      e.preventDefault();
      var elem = "";
...
if (almedisNewPassSubmit) {
  almedisNewPassSubmit.addEventListener(
    "click",
    function (e) {
      e.preventDefault();
      var elem = "";
...
if (almedisUserTabLinks) {
  for (let index = 0; index < almedisUserTabLinks.length; index++) {
    almedisUserTabLinks[index].addEventListener(
      "click",
      function (e) {
        e.preventDefault();
...
if (almedisPedidosLink) {
  for (let index = 0; index < almedisPedidosLink.length; index++) {
    almedisPedidosLink[index].addEventListener(
      "click",
      function (e) {
        e.preventDefault();
  ...
  for (let index = 0; index < elemPayment.length; index++) {
    elemPayment[index].addEventListener("click", function () {
      ...
      if (paymentType == "tarjeta") {
        document
          .getElementsByClassName("text-non-tarjeta")[0]
          .classList.add("hidden");
        document
          .getElementsByClassName("text-tarjeta")[0]
  ...
  paymentSubmit.addEventListener(
    ...
    function (e) {
      ...
      if (passd == true) {
        var newRequest = new XMLHttpRequest();
        var formData = new FormData(
          document.getElementById("almedisPaymentForm")
        );
        if (paymentSelected == "copago") {
        ...
        if (documentSelected == "no") {
          var fileCarta = document.getElementById(
            "payment_cartapoder_file"
          ).files;
...
if (trackingSubmit) {
  trackingSubmit.addEventListener(
    "click",
    function (e) {
      e.preventDefault();
      var elem = "";
      ...
      if (passd == true) {
        ...
        resultDiv.innerHTML = '<div class="loader"></div>';
        var formData = new FormData(
          document.getElementById("almedisTrackingForm")
        );
...
if (testimonialSubmit) {
  testimonialSubmit.addEventListener(
    "click",
    function (e) {
      e.preventDefault();
      var elem = "";
      ...
      // Getting Testimonial Institucion
      elem = document.getElementById("test_institucion");
      passd = almedisForms.validateInput(elem, "text");
...
jQuery(document).on("click", "#docsSubmit", function (e) {
  ...
  if (passd == true) {
    var pedidoId = document
      .getElementById("almedisModalForm")
      .getAttribute("data-pedido");
    var newRequest = new XMLHttpRequest();
    ...
    formData.append(
      "client_recipe",
      document.getElementById("client_recipe_file").files[0]
    );
    formData.append(
      "client_carta",
      document.getElementById("client_carta_file").files[0]
    );
    formData.append(
      "client_carnet",
      document.getElementById("client_carnet_file").files[0]
    );
    newRequest.open("POST", custom_admin_url.ajax_url, true);
    newRequest.onload = function () {
      var result = JSON.parse(newRequest.responseText);
...
var repeatPedidoClicks = document.getElementsByClassName(
  "almedis-repeat-pedido-click"
);
